...
def constructTableau(TS,CL):
    ...
    '''for ver in TS.getVertexs():
        for v in tableau:
            if v.state == ver.state:
                continue
            else:
                v.addFormula(LTLNot(LTLState(ver.state)))'''
...
aaa = constructCL(LTLNext(LTLNext(LTLUntil(LTLState('a'),LTLState('b')))))
a = Vertex('a')
b = Vertex('b')
c = Vertex('c')
d = Vertex('d')
